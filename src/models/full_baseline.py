...
postFB_fj_mask = copy.deepcopy(fj_mask)
for i in range(N_TOPS):
    postFB_fj_mask = np.logical_and(postFB_fj_mask, _fjets_["index"] != selected_bqq["index"][:, i])

################################################
# 3) Fill SRqq events
fjets = ak.mask(_fjets_, postFB_fj_mask)
jets = ak.mask(_jets_, mask)
qq_fjets = fjets[fjets['Wtag']]
b_jets = jets[jets['btag']]
...
chi2_SRqq = ak.where(t_mask, ((t.mass - TOP_MASS) / TOP_SIGMA)**2, FILL_VALUE)


selected_SRqqfjets = ak.pad_none(SRqq_fjets[ak.argsort(SRqq_fjets.pt, axis=1, ascending=False)], N_TOPS, axis=1, clip=True)
